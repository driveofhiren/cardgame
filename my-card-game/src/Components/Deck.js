...
export const Deck = () => {
  ...
  useEffect(() => {
    ...
    client.onmessage = (message) => {
      ...
      if (data.action === 'dealCards' && !dealCardsCalled) {
        // Call the dealCards function when the "dealCards" action is received
        dealCards();
        setDealCardsCalled(true);
  ...
  const [dealCardsCalled, setDealCardsCalled] = useState(false);
  const dealCards = () => {
